...
export default function LCPPreload({ imageUrl }: LCPPreloadProps) {
  useEffect(() => {
    ...
    // ✅ HARD STOP: only allow real URLs
    ...
    if (!isValidImageUrl) {
      // silently ignore public_id or junk values
      return;
    }

    const encodedUrl = encodeURI(imageUrl);

    if (document.querySelector(`link[href="${encodedUrl}"]`)) return;
    ...
    return () => {
      const el = document.querySelector(`link[href="${encodedUrl}"]`);
